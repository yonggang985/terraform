...
// A node that implements it specifies that any references _from_ that node
// are to be resolved in a separate module instance, as returned by the
// ReferenceOutside method.
//
// References _to_ that node are still resolved within the node's own path,
// as returned by its implementation of GraphNodeSubPath.
//
...
type GraphNodeReferenceOutside interface {
	...
	// are resolved.
	ReferenceOutside() (referencePath addrs.ModuleInstance)
}
...
// referenceMapKey produces keys for the "references" map. "referrer" is the vertex
...
func (m *ReferenceMap) referenceMapKey(referrer dag.Vertex, addr addrs.Referenceable) string {
	...
	if !ok {
		...
		panic(fmt.Errorf("referenceableMapKey on vertex type %T which doesn't implement GraphNodeSubPath", sp))
	}
	...
	if outside, ok := referrer.(GraphNodeReferenceOutside); ok {
		...
		// it was declared.
		path = outside.ReferenceOutside()
	} else {
		// Vertex makes references to objects in the same package as where it
		// was declared.
		path = sp.Path()
	}
...
func NewReferenceMap(vs []dag.Vertex) *ReferenceMap {
	...
	for _, v := range vs {
		...
		for _, addr := range sp.Path().Ancestors()[1:] {
