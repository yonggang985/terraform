...
func parseVariableValues(vv map[string]UnparsedVariableValue, configs map[string]*configs.Variable) (terraform.InputValues, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	ret := make(terraform.InputValues, len(vv))

	for name, rv := range vv {
		var mode configs.VariableParsingMode
		config, declared := configs[name]
		if declared {
		...
		if !declared {
			...
			case terraform.VariableFromConfig, terraform.VariableFromFile:
				...
				diags = diags.Append(&hcl.Diagnostic{
					...
					Subject:  val.SourceRange.AsHCL().Ptr(),
				})
			case terraform.VariableFromEnvVar:
				...
				// configurations.
			case terraform.VariableFromCLIArg:
